...
namespace(:interpreter) do
  namespace(:ruby22) do
    ...
    task :sources do
      ...
      when ENV['LOCAL']
      when ENV['CHECKOUT']
        Rake::Task['interpreter:ruby22:checkout'].invoke
      else
        Rake::Task['interpreter:ruby22:download'].invoke
      end
    end
    ...
    task :extract => [:extract_utils] do
      ...
      when ENV['CHECKOUT']
        ...
        files = Rake::Task['interpreter:ruby22:download'].prerequisites
...
desc "compile Ruby 2.2"
task :ruby22 => [
  'interpreter:ruby22:sources',
  'interpreter:ruby22:extract',
  'interpreter:ruby22:prepare',
  'interpreter:ruby22:configure',
  'interpreter:ruby22:compile',
  'interpreter:ruby22:install'
]

namespace :ruby22 do
  task :dependencies => ['interpreter:ruby22:dependencies']
  task :clean => ['interpreter:ruby22:clean']

  task :sh => ['interpreter:ruby22:dependencies'] do
    Rake::Task['devkit:sh'].invoke
  end
end

# Add RubyGems operating system customization
task :ruby22 => ['tools:rubygems:hook22']

# add Pure Readline to the chain
task :ruby22 => [:rbreadline]
task :ruby22 => ['dependencies:rbreadline:install22']

# add tcl/tk installation
unless ENV["NOTK"]
  task :ruby22 => ["dependencies:tk:install22"]
end

task :check22   => ['ruby22:dependencies', 'interpreter:ruby22:check']
task :irb22     => ['ruby22:dependencies', 'interpreter:ruby22:irb']
