...
namespace(:interpreter) do
  namespace(:ruby20) do
    ...
    task :sources do
      ...
      when ENV['LOCAL']
      when ENV['CHECKOUT']
        Rake::Task['interpreter:ruby20:checkout'].invoke
      else
        Rake::Task['interpreter:ruby20:download'].invoke
      end
    end
    ...
    task :extract => [:extract_utils] do
      ...
      when ENV['CHECKOUT']
        ...
        files = Rake::Task['interpreter:ruby20:download'].prerequisites
    ...
    task :configure => [package.build_target, :compiler, :dependencies] do
      ...
      unless uptodate?(File.join(package.build_target, 'Makefile'), [File.join(package.target, 'configure')])
        ...
        cd package.build_target do
          ...
          package.configure_options.push('DLDFLAGS=-static-libgcc') if compiler.version =~ /(?:32|64)\-4\.(?:8|9)\.?\d{0,2}\z/
...
desc "compile Ruby 2.0.0"
task :ruby20 => [
  'interpreter:ruby20:sources',
  'interpreter:ruby20:extract',
  'interpreter:ruby20:prepare',
  'interpreter:ruby20:configure',
  'interpreter:ruby20:compile',
  'interpreter:ruby20:install'
]

namespace :ruby20 do
  task :dependencies => ['interpreter:ruby20:dependencies']
  task :clean => ['interpreter:ruby20:clean']

  task :sh => ['interpreter:ruby20:dependencies'] do
    Rake::Task['devkit:sh'].invoke
  end
end

# Add RubyGems operating system customization
task :ruby20 => ['tools:rubygems:hook20']

# add Pure Readline to the chain
task :ruby20 => [:rbreadline]
task :ruby20 => ['dependencies:rbreadline:install20']

# add tcl/tk installation
unless ENV["NOTK"]
  task :ruby20 => ["dependencies:tk:install20"]
end

task :check20   => ['ruby20:dependencies', 'interpreter:ruby20:check']
task :irb20     => ['ruby20:dependencies', 'interpreter:ruby20:irb']
