...
namespace(:interpreter) do
  namespace(:ruby_trunk) do
    package = RubyInstaller::Ruby_trunk
    ...
    task :checkout => "downloads" do
      cd RubyInstaller::ROOT do
        ...
        if File.exist?(File.join(RubyInstaller::ROOT, package.checkout_target, '.git'))
          ...
          di = File.dirname(File.join(RubyInstaller::ROOT, package.checkout_target))
          Dir.mkdir(di) unless File.directory?(di)
          cd di do
            sh "git clone #{package.checkout} #{File.basename(package.checkout_target)}"
          end
        end
      end
    end
    ...
    task :sources do
      ...
      when ENV['LOCAL']
        ...
        Rake::Task['interpreter:ruby_trunk:checkout'].invoke
      end
    end
    ...
    task :compile_latest do
      if ENV['DKVER'].split('-')[2] < '4.7'
        msg=<<-EOS
        ENV['DKVER']=#{ENV['DKVER']} is too old.\n
        please choose one of below and gcc version >= 4.7:\n
        #{DevKitInstaller::COMPILERS.keys.sort.reverse}\n
        EOS
        fail msg
      end
    end
...
desc "compile Ruby trunk"
task :ruby_trunk => [
  'interpreter:ruby_trunk:compile_latest',
  'interpreter:ruby_trunk:sources',
  'interpreter:ruby_trunk:prepare',
  'interpreter:ruby_trunk:configure',
  'interpreter:ruby_trunk:compile',
  'interpreter:ruby_trunk:install'
]

namespace :ruby_trunk do
  task :dependencies => ['interpreter:ruby_trunk:dependencies']
  task :clean => ['interpreter:ruby_trunk:clean']

  task :sh => ['interpreter:ruby_trunk:dependencies'] do
    Rake::Task['devkit:sh'].invoke
  end
end

# Add RubyGems operating system customization
task :ruby_trunk => ['tools:rubygems:hooktrunk']

# add Pure Readline to the chain
task :ruby_trunk => [:rbreadline]
task :ruby_trunk => ['dependencies:rbreadline:installtrunk']

# add tcl/tk installation
unless ENV["NOTK"]
  task :ruby_trunk => ["dependencies:tk:installtrunk"]
end

task :checktrunk   => ['ruby_trunk:dependencies', 'interpreter:ruby_trunk:check']
task :irbtrunk     => ['ruby_trunk:dependencies', 'interpreter:ruby_trunk:irb']
